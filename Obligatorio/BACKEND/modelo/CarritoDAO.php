...
function realizarCompra($cantidad, $id_repuesto, $id_pedido, $metodo_pago, $id_cliente){
    $sql = "INSERT INTO pedido( fecha, metodo_pago, id_cliente) VALUES (0, CURRENT_DATE ,$metodo_pago,$id_cliente)"; 

    $connection = connection();
    $respuesta = $connection->query($sql);
    if($respuesta){
        $detalle=$this->agregarDetalle($id_repuesto, $id_pedido);
        if ($detalle){
...
function agregarDetalle($id_pedido){
    $sql = "SELECT
    p.id_pedido,
    p.fecha,
    p.metodo_pago,
